...
const _startPreloading = async () => {
  ...
  if (_isPreloaderEnabled.value) {
    feedbackView.value.setLoaderListeners(
      _onPreloaderShown,
      _onPreloadCompleted
    );
    feedbackView.value.setLoader("images/logo/dev-logo.webp");
  } else {
...
const _checkLoadProgress = () => {
  ...
  if (imageCount.total > 0) {
    imageLoadProgress = Math.round(
      (100 * imageCount.loaded) / imageCount.total
    );
