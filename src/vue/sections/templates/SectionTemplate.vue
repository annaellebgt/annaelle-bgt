<template>
  ...
  <section
    ...
    :class="props.sectionData['class']"
    :id="props.sectionData['id']"
  >
    <!-- Background -->
    <div class="bg-promo-image">
      <div class="bg-promo-image-overlay-faded"></div>
    </div>
    ...
    <div class="container-xxl">
      ...
      <!-- Section Header -->
      <div
        v-if="sectionHeadlineData && sectionHeadlineData['title']"
      ...
      <!-- Section Footer -->
      <div
        v-if="sectionFooterData && sectionFooterData['title']"
      ...
      >
        <div class="row h-100">
          <div class="col-12 col-lg-8 text-center mx-auto">
            <!-- Title -->
            <hr
              v-if="sectionFooterData['divider']"
              class="solid-divider mb-3"
            />
            <h3 v-html="sectionFooterData['title']" />

            <!-- Description -->
            <p
              v-if="sectionFooterData['description']"
              v-html="sectionFooterData['description']"
              class="mt-4 mb-2 text-4"
              :class="{
                'text-2 text-muted': sectionFooterData['smallDescription'],
              }"
            />

            <!-- Button -->
            <XLButton
              v-if="sectionFooterData['button']"
...
const downloadFile = () => {
  window.open(props.sectionData["footer"]["button"]["filePath"], "_blank");
};
</script>
